  ...
  return (
    ...
    >
      <div className="md:col-span-2 bg-card rounded-3xl border border-card-border p-5 shadow-sm">
            ...
            return (
              <button
                ...
                className={`relative flex flex-col items-center justify-center h-9 rounded-full text-sm font-medium transition-all ${
                  isSelected ? "bg-primary text-white" : isTod ? "bg-primary/10 text-primary" : "hover:bg-muted text-foreground"
                }`}
        ...
        {selectedDate && (
          <button onClick={() => setSelectedDate(null)} className="mt-3 w-full text-xs text-muted-foreground hover:text-foreground transition-colors">
            Xem tất cả sự kiện
          </button>
        )}
      </div>
      ...
      <div className="md:col-span-3 space-y-3">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-sm font-medium text-foreground">
            {selectedDate ? `Sự kiện ngày ${format(selectedDate, "d MMMM", { locale: vi })}` : "Sự kiện sắp diễn ra"}
          </span>
          {!selectedDate && (
            <span className="ml-auto bg-primary/10 text-primary text-xs px-2 py-0.5 rounded-full font-medium">{filteredEvents.length} sự kiện</span>
          )}
        </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1 mt-1 text-muted-foreground text-xs">
                    <Tag size={11} />
                    <span>{format(event.date, "d MMM", { locale: vi })}{!isSameDay(event.date, event.endDate) && ` — ${format(event.endDate, "d MMM", { locale: vi })}`}</span>
                  </div>
                </div>
              </motion.div>
            ))
      ...
      <div className="md:col-span-1 space-y-0 relative">
        ...
        {events.map((event, i) => (
          <motion.div
            ...
            className={`relative pl-14 pr-4 py-4 cursor-pointer rounded-2xl transition-colors ${selected === event.id ? "bg-primary/5 border border-primary/20" : "hover:bg-muted/50"}`}
            data-testid={`timeline-event-${event.id}`}
          >
            <div className={`absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 rounded-full bg-gradient-to-br ${event.accent} flex items-center justify-center shadow-md z-10`}>
              <span className="w-2 h-2 bg-white rounded-full" />
            </div>
            <p className="text-xs text-muted-foreground mb-0.5">{format(event.date, "d MMM", { locale: vi })}</p>
          ...
          {selected !== null ? (() => {
            ...
            return (
              <motion.div
                ...
                className="rounded-3xl overflow-hidden shadow-lg bg-card border border-card-border h-full"
              >
                <div className="relative h-52">
                  <img src={ev.image} alt={ev.title} className="w-full h-full object-cover" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" />
                  <div className="absolute bottom-0 left-0 right-0 p-5">
                    <span className={`inline-block text-xs px-2.5 py-0.5 rounded-full font-medium ${ev.categoryColor} mb-2`}>{ev.category}</span>
                    <h3 className="text-white font-bold text-xl leading-tight">{ev.title}</h3>
                  </div>
                </div>
                <div className="p-5 space-y-3">
                  ...
                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    ...
                    <span>{format(ev.date, "d MMMM yyyy", { locale: vi })}{!isSameDay(ev.date, ev.endDate) && ` — ${format(ev.endDate, "d MMMM yyyy", { locale: vi })}`}</span>
                  </div>
                  <button className="mt-4 w-full flex items-center justify-center gap-2 bg-primary text-white py-2.5 rounded-xl text-sm font-medium hover:bg-primary/90 transition-colors">
            ...
            <motion.div
              ...
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="flex flex-col items-center justify-center h-full min-h-64 text-muted-foreground rounded-3xl border-2 border-dashed border-border"
            >
              <CalendarDays size={40} className="mb-3 opacity-30" />
              <p className="text-sm">Chọn sự kiện để xem chi tiết</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-2 bg-muted rounded-full p-1">
            <button
              ...
              data-testid="button-events-option-a"
            >
              Lịch
            </button>
            <button
              ...
              data-testid="button-events-option-b"
            >
              Timeline
            </button>
          </div>
        ...
        <AnimatePresence mode="wait">
          {option === "A" ? (
            ...
            <motion.div key="B" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <TimelineView isInView={isInView} />
            </motion.div>
          )}
        </AnimatePresence>
      </div>
