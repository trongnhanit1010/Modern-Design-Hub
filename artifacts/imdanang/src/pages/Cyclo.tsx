...
function SpeakerPulse({ playing }: { playing: boolean }) {
  return (
    <div className="relative w-16 h-16 flex items-center justify-center">
      ...
      <div className="relative w-12 h-12 rounded-full bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center shadow-lg"
...
function TourCard({ tour, index }: { tour: typeof cycloTours[0]; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-3xl overflow-hidden shadow-sm border border-amber-100 hover:shadow-lg transition-shadow duration-300"
    >
      {/* Image */}
      <div className="relative h-64 sm:h-72 overflow-hidden">
        ...
        <div className="absolute top-4 right-4 flex items-center gap-1 bg-white/90 backdrop-blur text-amber-700 text-[11px] font-bold px-2.5 py-1 rounded-full">
          <Star size={10} className="fill-amber-500 text-amber-500" /> {tour.rating}
        </div>

        {/* Title overlay */}
        <div className="absolute bottom-4 left-4 right-20">
          <p className="text-amber-300 text-[10px] font-bold uppercase tracking-widest mb-1">{tour.tagline}</p>
      ...
      <div className="p-5">
        {/* Audio preview row */}
        <div className="flex items-center gap-3 rounded-2xl border border-amber-100 bg-amber-50 px-3 py-2.5 mb-4">
          <button
            onClick={() => setPlaying((p) => !p)}
            className="shrink-0 w-9 h-9 rounded-full bg-gradient-to-br from-amber-500 to-orange-500 text-white flex items-center justify-center hover:scale-105 transition-transform shadow"
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between text-[10px] mb-1">
              <span className="text-amber-600 uppercase tracking-widest font-bold">Audio guide preview</span>
              <span className="text-amber-400 font-mono">15 ngôn ngữ</span>
            </div>
            <Waveform playing={playing} bars={24} color="#d97706" />
          </div>
        </div>

        {/* Overview */}
        <p className="text-amber-900/70 text-sm leading-relaxed mb-4 line-clamp-3">
          {tour.overview}
        </p>
        ...
        <div className="grid grid-cols-1 gap-1.5 mb-4">
          {tour.highlights.map((h, i) => (
            <div key={i} className="flex items-start gap-2 text-sm">
              ...
              <span className="text-amber-900/80">{h}</span>
            </div>
          ))}
        </div>
        ...
        <button
          ...
          className="w-full flex items-center justify-between px-4 py-2.5 rounded-xl border border-amber-200 bg-amber-50 text-sm font-semibold text-amber-700 hover:bg-amber-100 transition-colors mb-4"
        >
                ...
                <div className="space-y-3">
                  {tour.stops.map((stop, i) => (
                    <div key={stop.id} className="flex gap-3 relative">
                      <div className={`shrink-0 w-3.5 h-3.5 rounded-full border-2 mt-1 z-10 ${i === 0 || i === tour.stops.length - 1 ? "border-amber-500 bg-amber-500" : "border-amber-300 bg-white"}`} />
                      <div>
                        <p className="text-sm font-semibold text-amber-900">{stop.name}</p>
                        <p className="text-xs text-amber-700/60 mt-0.5">{stop.description}</p>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-amber-100">
          <div>
            {tour.price ? (
              <>
                <div className="text-amber-700 text-2xl font-black">
                  {tour.price.toLocaleString("vi-VN")}đ
                </div>
                <div className="text-amber-500/70 text-[11px]">/ khách</div>
              </>
            ) : (
              <div className="text-amber-700 text-sm font-semibold">{tour.priceNote}</div>
            )}
          </div>
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="relative overflow-hidden border-b border-gray-100">
        <svg className="absolute inset-x-0 bottom-0 w-full h-24 opacity-30" viewBox="0 0 1200 100" preserveAspectRatio="none">
          <motion.path
            ...
            stroke="#d97706" strokeWidth="2" strokeDasharray="8 8" fill="none"
          ...
          <div className="grid lg:grid-cols-[1.2fr,1fr] gap-8 items-center">
            ...
            <div>
              ...
              <motion.h1 initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
                ...
                <span className="block text-transparent bg-clip-text bg-gradient-to-r from-amber-600 via-orange-500 to-amber-700">Du Lịch</span>
              </motion.h1>
              ...
              <div className="flex items-center gap-4 mb-5 overflow-hidden">
                ...
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="overflow-hidden"><Waveform playing={heroPlaying} bars={20} color="#d97706" /></div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-x-5 gap-y-2 text-amber-800/70 text-sm">
                ...
                <span className="flex items-center gap-1.5"><Headphones size={14} className="text-amber-600" /> 15 ngôn ngữ</span>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, rotate: 2 }} animate={{ opacity: 1, scale: 1, rotate: 0 }}
              ...
              <div className="relative rounded-3xl p-6 border border-amber-200 bg-white shadow-xl"
                ...
                <div className="rounded-2xl p-4 mb-4 border border-amber-100 bg-amber-50">
                  <div className="text-amber-500 text-[10px] uppercase tracking-widest mb-1">Điểm dừng 02 / 05</div>
                  ...
                  <div className="mt-3 space-y-1">
                    <div className="h-1 rounded-full bg-amber-100 overflow-hidden">
                      <motion.div className="h-full bg-gradient-to-r from-amber-500 to-orange-500"
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-amber-900 text-2xl font-black" style={{ fontFamily: "Georgia, serif" }}>
              {cycloTours.length} Hành trình có sẵn
            </h2>
            <p className="text-amber-700/60 text-sm mt-1">Hoạt động hàng ngày · Có audio guide tự động</p>
          </div>
        </div>
      ...
      {/* ─── AUDIO GUIDE ADD-ON ────────────────────────────── */}
      <section className="bg-gradient-to-br from-amber-950 to-amber-900 py-14 px-4">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-start gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-amber-300/70 text-sm">Nâng cấp trải nghiệm — Audio guide đa ngôn ngữ theo GPS</p>
            </div>
          </div>

          <p className="text-amber-200/80 text-sm leading-relaxed mb-6 max-w-2xl">
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {audioGuideLanguages.map((lang) => (
                <span key={lang} className="px-3 py-1 rounded-full bg-amber-500/15 border border-amber-500/25 text-amber-300 text-xs font-medium">
                  {lang}
                </span>
              ))}
            </div>
          ...
          <div className="grid sm:grid-cols-3 gap-3 text-sm">
            {[
              ...
              <div key={label} className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4">
                <Icon size={18} className="text-amber-400 mb-2" />
                <p className="text-white font-semibold text-sm">{label}</p>
                <p className="text-amber-300/60 text-xs mt-1">{desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6">
            <h3 className="text-amber-900 font-bold text-lg mb-4 flex items-center gap-2">
              <Phone size={18} className="text-amber-600" /> Liên hệ đặt tour
            </h3>
            <div className="space-y-3">
              {cycloContacts.map((c) => (
                <a key={c.phone} href={`tel:${c.phone.replace(/\./g, "")}`}
                  ...
                  <span className="text-amber-900 font-medium text-sm">{c.name}</span>
                  <span className="text-amber-600 font-mono text-sm font-bold group-hover:text-amber-700">{c.phone}</span>
                </a>
              ))}
            </div>
            <p className="text-amber-600/70 text-xs mt-3 text-center">Hoạt động 8:00 – 21:00 hằng ngày</p>
          </div>

          {/* Parking */}
          <div className="bg-orange-50 border border-orange-200 rounded-2xl p-6">
            <h3 className="text-amber-900 font-bold text-lg mb-4 flex items-center gap-2">
              <MapPin size={18} className="text-orange-500" /> Điểm đón khách
            </h3>
            <div className="space-y-3">
              {cycloParking.map((spot, i) => (
                <div key={i} className="flex gap-3 text-sm">
                  <div className="shrink-0 w-6 h-6 rounded-full bg-orange-100 border border-orange-200 flex items-center justify-center text-orange-600 text-xs font-bold">{i + 1}</div>
                  <p className="text-amber-900/80 leading-relaxed">{spot}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Note */}
        <div className="mt-6 flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-xl text-sm">
          <Coffee size={18} className="text-amber-600 shrink-0 mt-0.5" />
          <div>
            <span className="font-semibold text-amber-900">Lưu ý:</span>
            <span className="text-amber-800/70"> Tour Dạo ven sông Hàn hoạt động từ 8h00 đến 21h00 hằng ngày. Các tour Dấu Ấn Thời Gian và Hương Vị Việt hoạt động từ 8h00 đến 15h30. Vui lòng liên hệ trước để đặt lịch.</span>
          </div>
        </div>
      </section>
    </div>
