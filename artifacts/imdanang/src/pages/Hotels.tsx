...
export default function Hotels() {
  ...
  return (
    <CategoryShell
      ...
      sort={sort}
      setSort={setSort}
      resultCount={<><span className="text-white font-semibold">{filtered.length}</span> chỗ nghỉ phù hợp</>}
    >
                ...
                <div className="relative h-52 overflow-hidden">
                  ...
                  <div className="absolute bottom-3 left-3 flex gap-0.5">
                    {[...Array(hotel.stars)].map((_, s) => (
                      <Star key={s} size={11} style={{ color: acc.orbC }} className="fill-current" />
                    ))}
                  </div>
                </div>
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-1.5 mb-4 flex-wrap">
                    {hotel.amenities.map((a) => {
                      ...
                      return Icon ? (
                        <div key={a} className="flex items-center gap-1 px-2 py-1 bg-white/5 border border-white/10 rounded-lg">
                          <Icon size={11} className="text-white/55" />
                          <span className="text-white/55 text-[10px]">{amenityLabels[a]}</span>
                        </div>
                      ) : null;
                    })}
                  </div>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <span className="text-white/45 text-xs">/đêm</span>
                    </div>
                    <motion.div
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.96 }}
                      className="flex items-center gap-1 text-white text-sm font-semibold px-4 py-2 rounded-xl shadow-lg"
                      style={{
      ...
      {hasMore && (
        <div className="flex flex-col items-center gap-3 pt-10">
          <p className="text-white/50 text-sm">
            ...
            <span style={{ color: acc.orbC }} className="font-semibold">{remaining}</span> chỗ nghỉ
          </p>
          <motion.button
      ...
      {!hasMore && filtered.length > 0 && (
        <div className="pt-10 text-center text-white/45 text-sm">
          Đã hiển thị tất cả <span className="text-white/80 font-semibold">{filtered.length}</span> khách sạn
        </div>
      )}

      {filtered.length === 0 && (
        <div className="pt-16 flex flex-col items-center justify-center text-center">
          <Hotel size={40} className="text-white/30 mb-3" />
          <p className="text-white/80 font-medium">Không tìm thấy chỗ nghỉ phù hợp</p>
